...
$(function(){
	$.ajax({
		url: basePath+"/system/users/get",
		type : "POST",
		dataType : "json",
		data : {id:updateId}, //将Json对象序列化成Json字符串，toJSON()需要引用jquery.json.min.js
		success : function(res) {
			if(res.data){
				jQuery.json2form(res.data, "updateForm");
			}
		},
		error : function(res) {
...
function formSave(){
	if(checkForms()){
		...
		$.ajax({
			url: basePath+"/system/users/update",
			type : "POST",
			dataType : "json",
...
<body>
	<div class="easyui-panel" title="用户信息修改" style="width: 96%">
		<form id="updateForm" method="post">
			<input type="hidden" name="id" id="id" value="<%=updateId%>"/>
			<table width="100%">
				<colgroup>
					<col width="30%" />
					<col width="70%" />
				...
				<tbody>
					...
					<tr>
						<th colspan="2"><input type="button" value="修改" onclick="formSave()"/></th>
					</tr>
				</tbody>
			</table>
		</form>
